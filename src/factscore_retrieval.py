...
class Retrieval(object):
    ...
    def load_encoder(self):
        ...
        encoder = SentenceTransformer("sentence-transformers/" + self.retrieval_type, cache_folder='/'.join(self.cache_path.split('/')[:-1]) if '/' in self.cache_path else '.')
...
if __name__ == '__main__':
    ...
    data_path = ''
    cache_path = 'factcheck_cache/retrieval-enwiki-20230401-bm25.json'
    embed_cache_path = 'factcheck_cache/retrieval-enwiki-20230401-bm25.pkl'
    batch_size = 256
    database = DocDB(db_path=db_path, data_path=data_path)
    retriever = Retrieval(database, cache_path, embed_cache_path, batch_size=batch_size, retrieval_type='bm25')
    psgs = retriever.get_passages("Suthida", "He was born in 1988", k=5)
    print(psgs)
